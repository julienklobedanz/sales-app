...
const SmartMatchShell = dynamic(
  () => import('@/app/dashboard/smart-match/smart-match-shell').then((m) => m.SmartMatchShell),
  {
    ssr: false,
    loading: () => (
      <p className="px-4 py-8 text-sm text-muted-foreground">Smart Match wird geladen …</p>
    ),
  }
)
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-2xl lg:max-w-3xl">
        <SheetHeader className="border-b pb-4">
          ...
          <SheetDescription>{COPY.deals.cockpit.findReferenceDescription}</SheetDescription>
        </SheetHeader>
