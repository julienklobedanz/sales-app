...
export function LinkReferenceDialog({
  ...
  const [loadedRefs, setLoadedRefs] = useState<Array<{ id: string; title: string; company_name: string }> | null>(
    availableRefsProp ?? null
  )
  ...
  const disabled = useMemo(() => !loadingRefs && availableRefs.length === 0, [availableRefs.length, loadingRefs])
