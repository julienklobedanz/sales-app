...
describe('computeDeliveryWinProbability', () => {
  it('kombiniert Portfolio, Capabilities und Nachweise ohne KI-Score', () => {
    const breakdown = computeDeliveryWinProbability({
      requirements: [
        ...
        { id: 'r3', text: 'ISO 27001 Zertifizierung erforderlich', category: 'Compliance' },
      ],
      coverage: [
        {
          requirementId: 'r1',
          requirementText: 'Cloud Hosting in EU',
          matches: [{ id: 'a', title: 'Ref', summary: null, industry: null, similarity: 0.72, companyName: 'X' }],
        },
        {
          requirementId: 'r2',
          requirementText: 'SAP Schnittstelle',
          matches: [{ id: 'b', title: 'Ref2', summary: null, industry: null, similarity: 0.6, companyName: 'Y' }],
        },
        {
          requirementId: 'r3',
  ...
  it('erkennt Compliance-Anforderungen', () => {
    expect(
      isComplianceRequirement({ text: 'Anbieter muss ISO 27001 nachweisen', category: 'Security' })
    ).toBe(true)
    expect(isComplianceRequirement({ text: 'API Integration REST', category: 'Tech' })).toBe(false)
  })

  it('prüft Nachweise nur mit hochgeladenem Dokument', () => {
    expect(
      isComplianceRequirementFulfilled(
        { text: 'ISO 27001 erforderlich' },
        [{ document_type: 'iso_27001', title: 'ISO', valid_until: null, file_storage_path: null }]
      )
    ).toBe(false)
    expect(
      isComplianceRequirementFulfilled(
