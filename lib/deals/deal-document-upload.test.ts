...
describe('canManageDealDocuments', () => {
  ...
  it('allows system admin', () => {
    expect(
      canManageDealDocuments(deal, 'other', 'admin', 'sales_rep')
    ).toBe(true)
  })

  it('allows account_manager function role org-wide', () => {
    expect(
      canManageDealDocuments(deal, 'other', 'member', 'account_manager')
    ).toBe(true)
  })

  it('allows assigned sales manager', () => {
    expect(
      canManageDealDocuments(deal, 'sales-1', 'member', 'sales_rep')
    ).toBe(true)
  })

  it('allows assigned account manager on deal', () => {
    expect(
      canManageDealDocuments(deal, 'am-1', 'member', 'sales_rep')
    ).toBe(true)
  })

  it('denies unrelated sales rep', () => {
    expect(
      canManageDealDocuments(deal, 'other-rep', 'member', 'sales_rep')
    ).toBe(false)
  })
})
...
describe('deal-document-upload', () => {
  ...
  it('builds storage path', () => {
    expect(
      buildDealDocumentStoragePath('org', 'deal', 'doc', 'file.pdf')
