-- Jobtitel/Signatur für öffentliche Share-Kontaktkarte (unter dem Namen auf /p/…)

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS job_title text;

COMMENT ON COLUMN public.profiles.job_title IS
  'Anzeige unter dem Namen auf Kundenlinks (Share-Seiten).';
...
BEGIN
  ...
  v_role := nullif(trim(coalesce(v_profile.job_title, '')), '');
