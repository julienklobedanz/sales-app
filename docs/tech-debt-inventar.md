...
| P1-6  | Accounts Naming                                             | Route `accounts/`, Code `company*` (~18 Dateien)                                | Schrittweise Rename (DB `companies` ok)                                        | L       | Welle 5 — **bewusst offen**                                                              |
...
| P2-2 | Knip unused files (triagiert, App-Code) | u. a. `company-detail-links-tab`, `deal-activity-card`, `references/columns`+`data-table`, Settings-Orphans (`invite-card`, `settings-form`, `settings-danger-zone`, tabs), `lib/auth/get-user-role`, `lib/dashboard/can-view-insights`, DealDesk-Orphans (`bid-team`, `demo-seed`, …), `ticket-status-badge`, … | Nach Import-Check löschen                                  | M       | neu — **✅ ~35 Dateien**                                       |
