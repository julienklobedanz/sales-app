...
| P1-10 | Legacy-Mapping                                              | `lib/roles/legacy-mapping.ts` ~30+ Importe                                      | UI-Labels → system/function; Mapping schrumpfen                                | M       | Welle 5 — Invite-RPCs ohne `p_role`/`role`-JSON; Rest: Onboarding-Team-UI + Dev-Cookie + Settings-Fallback via `legacyRoleToDimensions` |
