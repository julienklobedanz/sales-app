...
**Zweck:** Eine **allgemeine Dokumenten-Sektion** im Deal-Cockpit. Kern-Einsicht: **Die RFP ist nur _ein_ Dokumenttyp** — die Dokumentenliste wird die Heimat, „RFP analysieren" ist eine Aktion auf einem Dokument vom Typ _Ausschreibung_ (räumt das letzte Upload-Duplikat auf).
...
2. **Deal-Ebene, nicht RFP-Block.** Die „Dokumente"-Sektion sitzt bei Fakten/Aktivität (relevant für Standard- _und_ RFP-Deals), **nicht** im konditionalen RFP-Block.
...
| Randfall                                | Regel                                                                                                                                                                                  |
| --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Storage-Orphans**                     | Löschen (Doc _und_ Deal) muss Storage-Objekte entfernen. Deal-Delete-Action: erst `deal_documents.storage_path`-Liste `remove()`, dann Deal löschen. (FK-Cascade räumt nur DB-Zeilen.) |
