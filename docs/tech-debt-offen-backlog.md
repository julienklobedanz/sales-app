...
| **2** | E7 Schema-Sync / Drift-Gate | Remote vs. Repo-Migrationen; `looseSelect`-Löcher; CI-Migrations-Gate festziehen | M | Schützt das schon vorhandene `database.types.ts`-Fundament | [`arbeitspaket-schema-sync-e7.md`](./arbeitspaket-schema-sync-e7.md), [`schema-sync.md`](./schema-sync.md) |
...
| **4** | **P1-6** Accounts Naming | App/Lib: `company*` → `account*` wo Domäne „Account“; **DB `companies` bleibt** | L (3–5 PRs) | Letzter klarer Inventar-Welle-5-Block; rein mechanisch → nach Security/Typen, bevor noch größere Renames | Inventar P1-6; empfohlenes Schnittmuster unten |
...
1. **Detail-UI-Dateien** umbenennen (`company-detail-*` → `account-detail-*`) + Imports — kein API-Rename.  
2. **Grid** (`companies-grid*` → `accounts-grid*`).  
